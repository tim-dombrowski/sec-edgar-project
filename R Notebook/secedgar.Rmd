...
useragent = "Tim Dombrowski tdombrowski@umsl.edu"
```
...
cikmap_url = "https://www.sec.gov/files/company_tickers.json"
cikmap_resp = request(cikmap_url) |>
...
cikmap_data = cikmap_resp |>
  resp_body_string() |>
  fromJSON()

cikmap = do.call(rbind.data.frame, cikmap_data) |>
  as_tibble() |>
  mutate(
    cik_str = as.integer(cik_str),
    cik = sprintf("%010d", cik_str),
    ticker = toupper(ticker)
  ) |>
  select(ticker, cik, cik_str, title) |>
  as.data.frame()
```
...
target.tickers = c("ABNB", "LVS", "NVDA", "TSLA")
target.rows = cikmap[cikmap$ticker %in% target.tickers, ]
target.rows
```
